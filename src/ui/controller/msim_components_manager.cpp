...
namespace fac {
    ...
    std::vector<std::string> msim_components_manager::placeable_bit_ids(QString id) {
        ...
        QStringList const bits =
                shared::id_reader::get_instance().get_results_for(
                    ".*(B|b)it.*" + id.remove("comp_").toStdString() + ".*");
        return placeable_ids_for(bits);
    }

    std::pair<msim_connector *, msim_connector_widget *> msim_components_manager::get_or_create_connector(
        QString conn_id) {
    ...
    std::pair<msim_decoder *, msim_decoder_widget *>
    msim_components_manager::get_or_create_decoder(QString decoder_id) {
